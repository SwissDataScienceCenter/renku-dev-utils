...
kubectl get namespaces \
  ...
  while IFS=$'\t' read -r namespace timestamp labels; do
    ...
    exemption_key=$(echo "$EXEMPTION_LABEL" | cut -d'=' -f1)
    exemption_value=$(echo "$EXEMPTION_LABEL" | cut -d'=' -f2)

    debug_log "Checking exemption: key='$exemption_key', value='$exemption_value'"
    debug_log "Namespace labels: $labels"

    # Check if the exemption label key exists with the correct value in the JSON labels
    if [[ "$labels" == *"\"$exemption_key\":\"$exemption_value\""* ]] || [[ "$labels" == *"\"$exemption_key\":$exemption_value"* ]]; then
      echo "  → Namespace $namespace is exempt from cleanup (has exemption label), skipping"
      debug_log "Namespace has exemption label: $exemption_key=$exemption_value"
      continue
    fi
